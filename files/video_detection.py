...
#this slows the camera frames but runs perfectly after hosting anywhere
prev_frames_live = []
prev_frames_id_live = []
track_id_live = 0
frame_list_live = {}
tracking_object_live = {}
predicted_class_name_live = {}
count_live = 0
i_live = 0
def process_video_usingLiveCam(image_byte,LRCN_model, yolo_model, object_list, IMAGE_HEIGHT, IMAGE_WIDTH, SEQUENCE_LENGTH, CLASSES_LIST):
    try:
        global prev_frames_id_live,predicted_class_name_live,prev_frames_live,track_id_live,frame_list_live,tracking_object_live,count_live,i_live
        ...
        original_video_width = frame.shape[1]
        original_video_height = frame.shape[0]
        ...
        if i_live == 0:
            ...
            if count_live <= 2:
                for pt1 in cur_frames:
                    for pt2 in prev_frames_live:
                        distance = dis(pt1, pt2)
                        if distance < 40:
                            tracking_object_live[track_id_live] = pt1
                            curr_frames_id.append(track_id_live)
                            frame_list_live[track_id_live] = deque(maxlen=20)

                            x1, y1, x2, y2 = bigger_frame(pt1, original_video_width, original_video_height)
                            newFrame = frame[y1:y2, x1:x2]
                            resized_frame = cv2.resize(newFrame, (IMAGE_HEIGHT, IMAGE_WIDTH))
                            normalized_frame = resized_frame / 255
                            frame_list_live[track_id_live].append(normalized_frame)
                            track_id_live += 1
                            break
                prev_frames_live = cur_frames.copy()
            else:
                tracking_object_copy = tracking_object_live.copy()
                curr_frames_copy = cur_frames.copy()
                for id, pt2 in tracking_object_copy.items():
                    id_exists = False
                    for pt in curr_frames_copy:
                        ...
                        if distance < 40:
                            ...
                            resized_frame = cv2.resize(newFrame, (IMAGE_HEIGHT, IMAGE_WIDTH))
                            normalized_frame = resized_frame / 255
                            frame_list_live[id].append(normalized_frame)
                            ...
                            break
                    if not id_exists:
                        tracking_object_live.pop(id)
                        frame_list_live.pop(id)
                        if predicted_class_name_live.get(id) is not None:
                            predicted_class_name_live.pop(id)

                for pt in cur_frames:
                    tracking_object_live[track_id_live] = pt
                    curr_frames_id.append(track_id_live)
                    frame_list_live[track_id_live] = deque(maxlen=20)
            ...
            for id in curr_frames_id:
                frame_queue = frame_list_live[id]
                if len(frame_queue) == SEQUENCE_LENGTH:
                    predicted_labels_probabilities = LRCN_model.predict(np.expand_dims(frame_queue, axis=0))[0]
                    predicted_label = np.argmax(predicted_labels_probabilities)
                    if predicted_labels_probabilities[predicted_label] > 0.7:
                        name = CLASSES_LIST[predicted_label]
                        predicted_class_name_live[id] = name
                        font = cv2.FONT_HERSHEY_DUPLEX
                        x1, y1, x2, y2 = tracking_object_live[id]
                        cv2.putText(frame, name, (x1 + 6, y1 - 6), font, 1.0, (255, 0, 0), 1)
                    else : 
                        predicted_class_name_live[id] = ""
            prev_frames_id_live = curr_frames_id.copy()
        else:
            for id in prev_frames_id_live:
                font = cv2.FONT_HERSHEY_DUPLEX
                x1,y1,x2,y2 = tracking_object_live[id]
                name = ""
                if predicted_class_name_live.get(id) is not None:
                    name = predicted_class_name_live[id]
                cv2.putText(frame,name , (x1 + 6, y1 - 6), font, 1.0, (255, 0, 0), 1)
        i_live += 1
        if track_id_live >= 50000:
            track_id_live = 0
        # Convert processed frame to JPEG and Base64 encode
...
# for faster camera frames only runs on local host
